...
class TestSimulationScenarios:
    ...
    @pytest.mark.asyncio
    async def test_create_configure_run_simulation(self, unified_client):
        ...
        # Configure simulation if resources are available
        config_params = {}

        if suppliers_response.suppliers:
            config_params['supplier_ids'] = [suppliers_response.suppliers[0].supplier_id]

        if workers_response.workers:
            # Find a logist or use first worker
            logists_response = await unified_client.get_all_logists()
            if logists_response.logists:
                config_params['logist_id'] = logists_response.logists[0].worker_id
            elif workers_response.workers:
                config_params['logist_id'] = workers_response.workers[0].worker_id
    ...
    @pytest.mark.asyncio
    async def test_simulation_data_consistency(self, unified_client):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_simulations(self, unified_client):
